...
**NOTA:** gli screenshots sono in bassa qualità a causa dei ridimensionamenti necessari per adattarli alla pagina

![Struttura della pagina creata](media/screenshots/Main_Page.png)

In figura si può vedere la struttura creata: attraverso l'uso del layout a 12 colonne che è standard assodato del web, si è creata una barra superiore contenente il nome del sito ed il logo dell'azienda **ACME**, lo spazio per eventuali link appartenenti al menù di navigazione da inserire al momento di necessità ed infine un pulsante che si distingue grazie al colore dagli altri che consente di avviare la procedura d'acquisto del prodotto.
...
![Texture Rock](media/screenshots/texture_rocl.png)
...
- Le funzioni **click_show_toy(), click_show_metallic(), click_show_used() e click_show_enviroment()** hanno il solo scopo di ascoltare il click del pulsante e settare a true la variabile corrispondente consentendo l'avvio della procedura specificata precedentemente per visualizzare gli oggetti desiderati;
...
- **TextureHammer()** visualizza la versione del martello con la testa blu metallizzato ed il manico con le texture;
- **adjustBackground()** Si occupa di fare in modo che il colore di sfondo della scena sia consono in base al martello visualizzato, quindi chiaro se non c'è sfondo, scuro in presenza di stelle. Si occupa inoltre della rimozione del martello precedente evitando sovvraposizioni di mesh e quindi sovvraccarico di lavoro inutile a schermo;
- **loadTexture(file)** è la funzione definita a lezione per il caricamento delle texture;
- **createParticleSystem()** si occupa della creazione del particle system descritto;
- **Space()** si occupa invece della creazione della scena che posiziona il martello nello spazio ascoltando lo stato del pulsante switch e svolgendo azioni che si adattano a seconda delle richieste utente.
